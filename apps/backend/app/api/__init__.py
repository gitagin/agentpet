"""FastAPI route modules."""
from fastapi import APIRouter, Depends

from ..auth import require_bearer_token
from . import agent, chat, checkpoints, continuity, diagnostics, growth, habit_loop, memory, metrics, reflection, settings, tasks, today, tts, vaults, wiki

api_router = APIRouter(prefix="/api", dependencies=[Depends(require_bearer_token)])
api_router.include_router(vaults.router)
api_router.include_router(agent.router)
api_router.include_router(memory.router)
api_router.include_router(growth.router)
api_router.include_router(habit_loop.router)
api_router.include_router(tasks.router)
api_router.include_router(settings.router)
api_router.include_router(tts.router)
api_router.include_router(today.router)
api_router.include_router(chat.router)
api_router.include_router(checkpoints.router)
api_router.include_router(continuity.router)
api_router.include_router(diagnostics.router)
api_router.include_router(wiki.router)
api_router.include_router(metrics.router)
api_router.include_router(reflection.router)
