...
export class CubismRenderer_WebGL extends CubismRenderer {
  ...
  public getModelRenderTarget(index: number): CubismRenderTarget_WebGL {
    if (this._modelRenderTargets == null) {
      this._modelRenderTargets = new Array<CubismOffscreenRenderTarget_WebGL>();
    }
    if (this._modelRenderTargets[index] == null) {
      const renderTarget = new CubismOffscreenRenderTarget_WebGL();
      ...
      this._modelRenderTargets[index] = renderTarget;
    }
  ...
  public loadShaders(shaderPath: string = null): void {
    ...
    const shaderSets = shader._shaderSets;
    if (
      shaderSets == null ||
      shaderSets.length == 0 ||
  ...
  public doDrawModel(shaderPath: string = null): void {
    ...
    if (this._drawableClippingManager != null) {
      ...
      for (
        ...
        if (this._drawableMasks[i] == null) {
          this._drawableMasks[i] = new CubismRenderTarget_WebGL();
        }
        if (
    ...
    if (this._offscreenClippingManager != null) {
      ...
      for (
        ...
        if (this._offscreenMasks[i] == null) {
          this._offscreenMasks[i] = new CubismRenderTarget_WebGL();
        }
        if (
  ...
  public drawObjectLoop(lastFbo: WebGLFramebuffer): void {
    ...
    for (let i = 0; i < totalCount; ++i) {
      const rawOrder = renderOrder[i];
      const order =
        Number.isFinite(rawOrder) && rawOrder >= 0 && rawOrder < totalCount
          ? rawOrder
          : i;
    ...
    for (let i = 0; i < totalCount; ++i) {
      ...
      if (objectIndex == null || objectType == null) {
        continue;
      }
  ...
  public addOffscreen(offscreenIndex: number): void {
    ...
    if (this._offscreenList == null) {
      this._offscreenList = new Array<CubismOffscreenRenderTarget_WebGL>();
    }
    if (this._offscreenList[offscreenIndex] == null) {
      ...
      try {
        this.setupParentOffscreens(this._model, this._model.getOffscreenCount());
      } catch (error) {
        console.warn('[Cubism] setupParentOffscreens skipped during lazy offscreen initialization.', error);
      }
    }
  ...
  public drawOffscreenWebGL(
    ...
    if (this._currentFbo == null) {
      this._currentOffscreen = this.getModelRenderTarget(0);
      this._currentFbo = this.getModelRenderTarget(0).getRenderTexture();
  ...
  public beforeDrawModelRenderTarget(): void {
    if (this._modelRenderTargets == null || this._modelRenderTargets.length == 0) {
      return;
    }
    ...
    // 別バッファに描画を開始
    this.getModelRenderTarget(0).beginDraw();
    this.getModelRenderTarget(0).clear(0.0, 0.0, 0.0, 0.0);
  }
  ...
  public afterDrawModelRenderTarget(): void {
    ...
    // 元のバッファに描画する
    this.getModelRenderTarget(0).endDraw();
  ...
  getOffscreenMaskBuffer(index: number): CubismRenderTarget_WebGL {
    ...
    if (this._offscreenMasks[index] == null) {
      const maskBuffer = new CubismRenderTarget_WebGL();
      const maskSize =
        this._offscreenClippingManager != null
          ? this._offscreenClippingManager.getClippingMaskBufferSize()
          : 256;
      maskBuffer.createRenderTarget(this.gl, maskSize, maskSize, this._currentFbo);
      this._offscreenMasks[index] = maskBuffer;
    }
  ...
  public getDrawableMaskBuffer(index: number): CubismRenderTarget_WebGL {
    ...
    if (this._drawableMasks[index] == null) {
      const maskBuffer = new CubismRenderTarget_WebGL();
      const maskSize =
        this._drawableClippingManager != null
          ? this._drawableClippingManager.getClippingMaskBufferSize()
          : 256;
      maskBuffer.createRenderTarget(this.gl, maskSize, maskSize, this._currentFbo);
      this._drawableMasks[index] = maskBuffer;
    }
