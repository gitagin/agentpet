...
describe("wiki terminology", () => {
  it("uses product language in the Knowledge Base shell", () => {
    ...
    expect(screen.getByText(/记忆工作台的一部分/)).toBeInTheDocument();
  ...
  it("prioritizes recent organized output and collapses maintenance status", () => {
    ...
    expect(within(maintenance).getByText("本机文件夹")).not.toBeVisible();
  ...
  it("shows no Vault, no pages, and index-required states clearly", () => {
    ...
    expect(screen.getAllByText(/未设置本机文件夹/).length).toBeGreaterThan(0);
  ...
  it("keeps advanced maintenance collapsed and risk-aware", () => {
    ...
    expect(screen.getByText(/部分操作会写入本地文本/)).toBeInTheDocument();
